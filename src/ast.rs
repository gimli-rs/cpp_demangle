...
macro_rules! try_begin_parse {
    ( $production:expr , $ctx:expr , $input:expr ) => {
        ...
        let _auto_check_recursion = try!(AutoParseRecursion::new($ctx));
    }
}
...
macro_rules! begin_demangle {
    ( $production:expr , $ctx:expr , $scope:expr ) => {
        let _log = AutoLogDemangle::new($production, $ctx, $scope, false);
        let _auto_check_recursion = try!(AutoParseDemangle::new($ctx));
    }
}

/// Automatically log start and end demangling in an s-expression format, when
/// the `logging` feature is enabled.
macro_rules! begin_demangle_as_inner {
    ( $production:expr , $ctx:expr , $scope:expr ) => {
...
/// maximum. If the maximum has been crossed, return an error. Otherwise,
/// increment the level upon construction, and decrement it upon destruction.
struct AutoParseDemangle(Rc<Cell<DemangleState>>);

impl AutoParseDemangle {
    #[inline]
    fn new<'a, W: 'a + DemangleWrite>(ctx: &DemangleContext<'a, W>) -> std::result::Result<AutoParseDemangle, fmt::Error> {
        ctx.enter_recursion()?;
        Ok(AutoParseDemangle(ctx.state.clone()))
    }
}

impl Drop for AutoParseDemangle {
    #[inline]
    fn drop(&mut self) {
    ...
    W: 'a + DemangleWrite,
{
    ...
    pub fn new(
        ...
        DemangleContext {
            ...
            state: Rc::new(Cell::new(DemangleState {
                recursion_level: 0,
            })),
        }
    }
    ...
    W: 'subs + DemangleWrite,
{
    fn demangle_as_inner<'prev, 'ctx>(
        ...
        begin_demangle_as_inner!(self, ctx, scope);
